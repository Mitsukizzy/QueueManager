...
void enqueue_byte(Q *q, unsigned char b)
{
    ...
    {
        data[q->back] = b;
        printf("1Added %u to queue #%d index %d\n", b, q->qNum, q->back );
        q->back -= 1;
    ...
    {        
        ...
        int nextQFront = get_int_from_chars(1 + 4 * q->qNum); 
        int freeQueueSlots = q->back - nextQFront;

        if(freeQueueSlots >= 1)
        {            
            data[q->back] = b;
            printf("2Added %u to queue #%d index %d\n", b, q->qNum, q->back );
            q->back -= 1;
...
{    
    ...
    Q *newestQ =  reinterpret_cast<Q*>(&data[headerLen - 4]);
    int dataBack = newestQ->back;
    int totalFree = dataBack - headerLen;

    Q *q = reinterpret_cast<Q*>(&data[headerLen - 4]);
    printf("OH PLS WORK - qNum: %u     back: %u     front: %u     q: %u\n", q->qNum, q->back, q->front, q );
...
int get_buffer_space()
{
    ...
    int dataFront = firstQ->front;
    int dataBack = lastQ->back;
